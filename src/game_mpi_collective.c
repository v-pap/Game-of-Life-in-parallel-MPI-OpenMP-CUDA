...
void game(int width, int height, char *fileArg)
{
    ...
    int err = MPI_File_open(new_comm, "./collective_output.out", MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL, MPI_INFO_NULL, &fh); //opening the file

    if (err != MPI_SUCCESS) // If the file already exists
    {
        if (me == 0) // Then the master process deletes it
            MPI_File_delete("./collective_output.out", MPI_INFO_NULL);
        MPI_File_open(new_comm, "./collective_output.out", MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_EXCL, MPI_INFO_NULL, &fh); // and the file is opened again
    }
